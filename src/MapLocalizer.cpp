...
void MapLocalizer::spin(const ros::TimerEvent& e)
{
  ...
  {
    currentKeyframe = new KeyframeContainer(img_undistort, img_match_descriptor_type);
  }
...
#else
    // ASift localization with local search refinement
...
Mat MapLocalizer::GetVirtualImageFromTopic(Eigen::Matrix4f tf, Mat& depths, Mat& mask)
{
  Mat vimg;
  ...
  // TODO: Fix the queue size, this is hacky 
  ros::topic::waitForMessage<sensor_msgs::Image>("virtual_image", nh);
  sensor_msgs::ImageConstPtr img_msg = ros::topic::waitForMessage<sensor_msgs::Image>("virtual_image", nh);
  sensor_msgs::ImageConstPtr depth_msg = ros::topic::waitForMessage<sensor_msgs::Image>("virtual_depth", nh);
  cv_bridge::CvImageConstPtr cvImg = cv_bridge::toCvCopy(img_msg);
  vimg = cvImg->image;
  ...
  for(int i = 0; i < virtual_height; i++)
  {
    for(int j = 0; j < virtual_width; j++)
    {
      ...
      int index = i*depth_msg->step + j*(depth_msg->step/depth_msg->width);
      for(int k = 0; k < 4; k++)
      {
        u.b[k] = depth_msg->data[index+k];
      }
...
bool MapLocalizer::LoadPhotoscanFile(std::string filename, std::string desc_filename, bool load_descs)
{
  ...
  if(load_descs)
  {
    ROS_INFO("Opening keypoints and descriptors...");
    //fs = FileStorage(desc_filename, FileStorage::READ);
    desc_file.open(desc_filename.c_str(), std::ifstream::binary);
    //if(!fs.isOpened())
    if(!desc_file.is_open())
    {
      ROS_ERROR("Could not open descriptor file %s", desc_filename.c_str());
      return false;
    }
    ROS_INFO("Successfully opened keypoints and descriptors");
  }
    ...
    chunk != NULL; chunk = chunk->NextSiblingElement("chunk"))
  {
	...
	img_in.release();
	  ...
	  desc_file.read((char*)data, sizeof(unsigned char)*rows*cols*elemSize);
        ...
        {  
          kfc = new KeyframeContainer(img_undistort, img_match_descriptor_type, tf, K);
        }
...
bool MapLocalizer::WriteDescriptorsToFile(std::string filename)
{
  ...
  if(!file.is_open())
  {
    ROS_ERROR("Could not open descriptor file %s", filename.c_str());
    return false;
  }

  ROS_INFO("Saving keypoints and descriptors...");
  for(unsigned int i = 0; i < keyframes.size(); i++)
  {
  ...
  file.close();
  ROS_INFO("Successfully saved keypoints and descriptors to %s", filename.c_str());
  return true;
  /*
...
bool MapLocalizer::FindImageTfVirtualPnp(KeyframeContainer* kfc, Eigen::Matrix4f vimgTf, Eigen::Matrix3f vimgK, Eigen::Matrix4f& tf)
{
  ...
  ROS_INFO("VirtualPnP: find keypoints time: %f", (ros::Time::now()-start).toSec());
  ...
  ROS_INFO("VirtualPnP: match time: %f", (ros::Time::now()-start).toSec());
...
std::vector< KeyframeMatch > MapLocalizer::FindImageMatches(KeyframeContainer* img, int k, bool usePos)
{
  ...
  if(usePos)
  {
    ROS_INFO("Peforming local match search...");
  ...
  {
    ROS_INFO("Performing global match search...");
  }
