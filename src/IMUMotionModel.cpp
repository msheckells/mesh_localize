...
void IMUMotionModel::monitorQueue()
{
  ...
  while (ros::ok())
  {
    ...
    if((ros::Time::now() - lastTime).toSec() > 0.02)
    {
      publishTf();
...
void IMUMotionModel::init(const Eigen::Matrix4f& tf, const Matrix6f& cov)
{
  ...
  ins_state.P.block<3,3>(3,3).diagonal() = gyro_bias_var;     // bg
  ins_state.P.block<3,3>(6,6).diagonal() = acc_bias_var;    // ba
...
Eigen::Matrix4f IMUMotionModel::predict()
{
  ...
  for(int i = 0; i < dt_store_copy.size(); i++)
  {
    ...
    //std::cout << "a["<<i<<"]="<<a_store[i].transpose() << std::endl;;
